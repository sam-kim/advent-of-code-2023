...
class Num
  attr_accessor :y_range, :x_range, :data, :chosen

  def initialize(y_range, x_range, data)
    @y_range = y_range
    @x_range = x_range
    @data = data
    @chosen = false
...
input.split("\n").each_with_index do |line, yd|
  line.each_char.each_with_index do |i, xd|
    POINTS << Point.new(xd, yd, i)
  end
end

ACTIONABLE = POINTS.reject { |p| p.data == "." }
NUMS = ACTIONABLE.select { |p| p.data.scan(/\d/) unless p.data.scan(/\d/)[0].nil? }
SYMBOLS = ACTIONABLE.reject { |p| p.data.scan(/\d/) unless p.data.scan(/\d/)[0].nil? }
POINTS_BUNCHED = NUMS.slice_when {|i, j| i.x+1 != j.x }.to_a.each do |part|
  y_range = (part.first.y - 1)..(part.last.y + 1)
  x_range = (part.first.x - 1)..(part.last.x + 1)
  PARTS << Num.new(y_range, x_range, part.map(&:data).join)
end

# PART 2
GEARS = ACTIONABLE.select { |p| p.data == "*" }

g= GEARS.map do |point|
  a = PARTS.map do |part|
    part if part.x_range === point.x && part.y_range === point.y
  end.compact
...
b = g.map do |nums|
  nums.map(&:data).map(&:to_i).inject(:*)
end.compact.sum
puts b

# PART 1
# SYMBOLS.each do |point|
#   PARTS.map do |part|
#     if part.x_range === point.x && part.y_range === point.y && !part.chosen
#       part.chosen = true
#       part
#     end
#   end
...
# puts PARTS.select(&:chosen).map(&:data).map(&:to_i).sum
